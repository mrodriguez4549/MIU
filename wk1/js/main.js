...
// toggle controls
	...
	// stores all object data
		...
		var item  				= {};
			...
			item.caramount		= ["Number of cars you own:", 	$('caramount').value];
			item.fastest		= ["Fastest Time:", 				$('fast').value];
			item.hp				= ["Horse Power:", 					$('hpr').value];
			item.adder			= ["Power Adder:", 					$('adders').value];
			item.tranny			= ["Transmission:", 				trannyValue];
			item.ltech			= ["Referred from LS1Tech.com:", 	siteValue];
			item.comments		= ["Any Comments:", 				$('comments').value];
	...
	function editItem(){
		...
		$('fast').value = item.fastest[1];
		$('hpr').value = item.hp[1];
		$('adders').value = item.adder[1];
		var radios = document.forms[0].tranny;
		for(var i = 0; i<radios.length; i++){
